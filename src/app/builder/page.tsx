          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
                {view === 'templates' ? (
                  <>Create New <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Resume</span></>
                ) : (
                  <>My <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Documents</span></>
                )}
              </h1>
            </div>

            <div className="flex p-1.5 bg-slate-100/80 backdrop-blur-md rounded-2xl border border-slate-200/60 shadow-inner">
              <button 
                onClick={() => setView('list')}
                className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${
                  view === 'list' ? 'bg-white text-blue-600 shadow-[0_4px_12px_rgba(0,0,0,0.05)] ring-1 ring-slate-200/50' : 'text-slate-500 hover:text-slate-900'
                }`}
              >
                My Resumes
              </button>
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {userResumes.map((resume) => (
                  <div 
                    key={resume.id}
                    className="group relative bg-white rounded-[32px] border border-slate-200/60 p-2.5 hover:border-blue-300 transition-all duration-500 hover:shadow-[0_20px_40px_-15px_rgba(37,99,235,0.15)]"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 to-transparent opacity-0 group-hover:opacity-100 rounded-[32px] transition-opacity duration-500" />
                    <Link href={`/builder/edit?id=${resume.id}${isOptimizeMode ? '&tab=optimize' : ''}`} className="block relative z-10">
                      <div className="aspect-[1/1.41] bg-white rounded-2xl mb-4 relative overflow-hidden border-2 border-slate-100 group-hover:border-blue-200 transition-all duration-500 shadow-sm group-hover:shadow-md">
                        ...
                        <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/10 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
                          <div className="w-14 h-14 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl flex items-center justify-center opacity-0 scale-50 group-hover:opacity-100 group-hover:scale-100 transition-all duration-500 delay-75 border border-white/50">
                            {isOptimizeMode ? <Target className="w-7 h-7 text-blue-600 drop-shadow-sm" /> : <ArrowUpRight className="w-7 h-7 text-blue-600 drop-shadow-sm" />}
                          </div>
                        </div>
                      </div>
                    </Link>
                    ...
                    >
                      <div className="w-full aspect-[1/1.4] bg-slate-100 rounded-[28px] border border-slate-200 overflow-hidden relative">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-slate-900/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-end pb-8">
                          <div className="px-8 py-3.5 rounded-2xl bg-white/95 backdrop-blur-sm text-blue-600 font-black shadow-2xl text-sm flex items-center gap-2 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
